...
function radius_node_id_scatter_chart (data) {
  ...
  function hoverFeature (event, d) {
    const hoverX = event.pageX + 10
    const hoverY = event.pageY - 10
    const latestClientString = getClientStringFromDecodedEnr(d.raw_enr)
    hover
      .html(
...
function getClientStringFromDecodedEnr (enr) {
  for (const [key, value] of ENR.ENR.decodeTxt(enr).enr.entries()) {
    if (key === 'c') {
      const fullClientString = String.fromCharCode.apply(null, value)
